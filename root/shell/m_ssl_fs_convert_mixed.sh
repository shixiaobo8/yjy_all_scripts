...
do
	...
	_log "开始生成加密版本切片"ssl_${file_name}.m3u8
	#切片
	file_url=${WEB_URL}${date}"/"${random_path}"/"${file_name}""_aes".m3u8"
	segmenter -y -i $file_name.ts -d 10 -p $file_name -m $file_name"_aes".m3u8  -u ${WEB_URL}${date}/${random_path}/ &>/dev/null
	_log "生成切片成功"
